...
float *IMU_read_data(){
    ...
    //For a range of +-2g, we need to divide the raw values by 16384, according to the datasheet **INCORRECT**
    float AccX = ((int16_t)(Wire.read() << 8 | Wire.read())) / 4096.0f;
    float AccY = ((int16_t)(Wire.read() << 8 | Wire.read())) / 4096.0f;
    float AccZ  = ((int16_t)(Wire.read() << 8 | Wire.read())) / 4096.0f;
    /*
    float AccX = (Wire.read() << 8 | Wire.read()) / 4096; // 16384.0; // X-axis value
    float AccY = (Wire.read() << 8 | Wire.read()) / 4096; // 16384.0; // Y-axis value
    float AccZ = (Wire.read() << 8 | Wire.read()) / 4096; // 16384.0; // Z-axis value
    */
    ...
    // === Update timestamps === //
    unsigned long currentTime = millis(); // Current time actual time read
    float elapsedTime = (currentTime - previousTime); // Calculate elapsed time
    previousTime = currentTime; // Previous time is stored before the actual time read
    RPYT[3] = currentTime;
    RPYT[4] = elapsedTime;
    elapsedTime = elapsedTime / 1000.0; // Converts back to seconds for calculation
    ...
    // For a 250deg/s range we have to divide first the raw value by 131.0, according to the datasheet **INCORRECT**
    float GyrX = ((int16_t)(Wire.read() << 8 | Wire.read())) / 32.8f;
    float GyrY = ((int16_t)(Wire.read() << 8 | Wire.read())) / 32.8f;
    float GyrZ = ((int16_t)(Wire.read() << 8 | Wire.read())) / 32.8f;
    /*
    float GyrX = (Wire.read() << 8 | Wire.read()) / 32.8; // 131.0; 
    float GyrY = (Wire.read() << 8 | Wire.read()) / 32.8; // 131.0;
    float GyrZ = (Wire.read() << 8 | Wire.read()) / 32.8; // 131.0;
    */
...
float  *IMU_error_calc() {  
    ...
    for(int i = 0; i < 200; i++){
        ...
        float AccZ = ((int16_t)(Wire.read() << 8 | Wire.read())) / 4096.0f;
        /*
        float AccX = (Wire.read() << 8 | Wire.read()) / 4096; // 16384.0 ;
        float AccY = (Wire.read() << 8 | Wire.read()) / 4096; // 16384.0 ;
        float AccZ = (Wire.read() << 8 | Wire.read()) / 4096; // 16384.0 ;
        */
        // Sum all readings
    ...
    for(int i = 0; i < 200; i++){
        ...
        */
        // Sum all readings
        errors[2] = errors[2] + (GyrX); // 131.0 // 32.8
    ...
    // Initialise Previous Time (as close as possible to loop start)
    previousTime = millis();
